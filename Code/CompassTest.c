/* LSM303DLH Example Code
   by: Jim Lindblom - Additions by Kevin Coble
   SparkFun Electronics
   date: 9/6/11 - Additions on 7/9/16
...
/* Global variables */
short accel[3];  // we'll store the raw acceleration values here
short mag[3];  // raw magnetometer values stored here
float realAccel[3];  // calculated acceleration values here

//  Calibration constants for compass
short mag_x_offset = 150;
short mag_y_offset = 55;
float mag_x_multiplier = 1.12;    //  Multiplier to equalize x axis range with y axis
float mag_declination = 12.083;   //  Local magenetic declination (subtracted from resulting angle)
...
void setup()
{
  Serial.begin(19200);  // Serial is used for debugging
  ...
  //  Set the compass calibration factors
  eeprom_values.mag_x_offset = 150;
  eeprom_values.mag_y_offset = 55;
  eeprom_values.mag_x_multiplier = 1.12;    //  Multiplier to equalize x axis range with y axis
  eeprom_values.mag_declination = 12.083;   //  Local magenetic declination (subtracted from resulting angle)
}
...
void loop()
{
 ...
 //!!  Uncomment the following line to see the magnetometer values
 // printValues(mag, accel);  // print the raw accel and mag values, good debugging
  ...
  int button;
  button = getButton();
  if (button == 1) {    //  S2 writes the eeprom
    write_eeprom();
    Serial.println("EEPROM updated");  // print some tabs
  }
...
float getHeading(short * magValue)
{
  ...
  heading -= eeprom_values.mag_declination;
...
void getLSM303_mag(short * rawValues)
{
  ...
  for (int i=0; i<3; i++)
   ...
   rawValues[0] += eeprom_values.mag_x_offset;
   rawValues[0] = (short)((float)rawValues[0] * eeprom_values.mag_x_multiplier);
   rawValues[1] += eeprom_values.mag_y_offset;
}
